...
local campaign_util = require("campaign_util")
local technology_manager = require("technology_manager")

local effects = {}

effects.unlock_entities_in_area = function(area_name)
...
effects.replace_compi_box = function(box_tag, goal_items)
  ...
  local replacement_ent = box_ent.surface.create_entity({name='wooden-chest',position=box_ent.position,force='player'})
...
effects.replace_consumer = function(consumer_tag, goal_items)
...
effects.take_items_from_player = function(itemlist)
...
effects.take_items_from_players = function(itemlist)
  for _, player in pairs(game.connected_players) do
    for index, item in pairs(itemlist) do
      local removed = player.remove_item({name=item.name,count=item.count})
      item.count = item.count - removed
  ...
  assert(#itemlist>0,"effect.take_items_from_players: not all items were able to be taken")
end

effects.refund_consumer = function(consumer_tag, goal_items)
...
effects.swap_tagged_with_fixed_entity = function(tag, entity_name, new_entity_type)
  ...
  local new_ent = locations.get_main_surface().create_entity({
      name=new_entity_type or entity_name,
...
effects.swap_tagged_with_fixed_recipe = function(tag, recipe_name, new_entity_type)
  local am = effects.swap_tagged_with_fixed_entity(tag,new_entity_type or 'assembling-machine-1')
...
effects.turn_biters_around = function(area_list, destination_name)
...
effects.get_entities_to_swap_forces = function(box, from, everything, only_units)
  ...
  for _, ent in pairs(froments) do
    if not (ent.name == 'character' or
            ...
            (ent.name == 'crash-site-assembling-machine-1-repaired' and protect_player_ents) or
            (ent.name == 'crash-site-assembling-machine-2-repaired' and protect_player_ents) or
            (ent.name == 'crash-site-lab' and protect_player_ents) or
            (ent.name == 'crash-site-generator' and protect_player_ents) or
...
effects.lock_structure_with_tag = function(tag)
  local target_ent = locations.get_surface_ent_from_tag(locations.get_main_surface(),tag)
  assert(target_ent,"effects: tried to lock a structure using an invalid entity tag "..tag)
  target_ent.force = 'pre-placed'
  target_ent.minable = false
  target_ent.destructible = false
  if target_ent.name == 'gate' then
    target_ent.active = true
  end
end

effects.swap_forces = function(box, from, to, everything, only_units)
  local entities = effects.get_entities_to_swap_forces(box, from, everything, only_units)
...
effects.activate_radar = function(state)
  for _, player in pairs(game.players) do
    player.minimap_enabled = state
  end
end

effects.move_player_out_of_area = function(box)
...
effects.activate_research = function(state)
  for _, player in pairs(game.players) do
    player.game_view_settings.show_research_info = state
  end
end

effects.activate_side_menu = function(state)
  for _, player in pairs(game.players) do
    player.game_view_settings.show_side_menu = state
  end
end

effects.activate_controller_gui = function(state)
  for _, player in pairs(game.players) do
    player.game_view_settings.show_controller_gui = state
  end
end

effects.activate_quick_bar = function(state)
  for _, player in pairs(game.players) do
    player.game_view_settings.show_quickbar = state
  end
end

effects.activate_shortcut_bar = function(state)
  for _, player in pairs(game.players) do
    player.game_view_settings.show_shortcut_bar = state
  end
end

effects.show_text_window = function(heading, blocks)
...
effects.unlock_technologies = function(tech_level, do_flying_text)
...
effects.unlock_optional_technologies = function(tech_level, do_flying_text)
  if do_flying_text == nil then
    do_flying_text = true
  end

  technology_manager.enable_optional_tech_level(tech_level)
  if do_flying_text then
    campaign_util.flying_tech_unlocked()
  end
end

effects.reveal_technologies = function(tech_level)
  technology_manager.set_revealed_tech_level(tech_level)
end

effects.discover_all_tech_levels = function()
  technology_manager.reveal_all_tech_levels()
end

effects.research_technologies = function(tech_level)
 technology_manager.research_up_to(tech_level)
end

effects.clone_area = function(area_name)
...
effects.expand_map = function(expansion_args, skip_cutscene)
...
effects.spawn_biters = function(quant, position, force_name, unit_type, as_group)
...
effects.send_sequenced_attack = function(spawn_name, quant, force, seconds_between, target_list)
...
effects.attack_ent_with_group_via_rally = function(group, target, rally, follow_up)
...
effects.turn_biters_hostile = function(area_name)
  -- within given area move biters to enemy force
  local toents = effects.swap_forces(locations.get_area(area_name),'peaceful','enemy', nil, true)
...
effects.make_peaceful_biters_wander = function(area_name)
  -- within given area move biters to enemy force
  local area = locations.get_area(area_name)
...
effects.swap_recipes = function(recipe1, recipe2)
...
effects.disable_recipes = function(recipelist)
  for _, recipe in pairs(recipelist) do
    game.forces.player.recipes[recipe].enabled = false
  end
end

effects.enable_recipes = function(recipelist)
  for _, recipe in pairs(recipelist) do
